...
**Input** | **Output**
--------- | ----------  
10        | 30 50 11.1803398874989
5         | 

**Input** | **Output**
--------- | ----------  
22.1 | 64.6 225.42 24.3402958075698
10.2

#### Hints

· Use Math.Sqrt() to calculate square root for calculating the diagonal (c
2 = a
2 + b

2).See [http://www.mathopenref.com/rectanglediagonals.html](http://www.mathopenref.com/rectanglediagonals.html).
...
You are given a program that checks if numbers in a given range [2...N] are prime. For each number is printed "{number} -&gt; {True or False}". The code however, is not very well written. Your job is to modify it in a way that is **easy to read and
understand.**
...
we assume two numbers are equal if they are more closely to each other than some fixed constant **eps**. 
...
If there is more than one appropriate data type, print each one on its own line and order them by size (sbyte**
...
**_This problem is from the _****_Programming Fundamentals Retake Exam (11 September
2016)._**
...
pictures that are considered "good". Then she needs to upload every single **filtered** picture to her cloud. She is considered ready when all **filtered** pictures are **uploaded **in her picture storage.
...
On the fourth line, you will receive an integer **UT** – the amount of time needed for every **filtered** picture to
